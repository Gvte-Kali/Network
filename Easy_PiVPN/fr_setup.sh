...
step8() {
  ...
  # Demander le message personnalisé
  read -p "Entrez un message personnalisé (optionnel) : " custom_message
  
  # Sauvegarder l'URL du webhook dans un fichier
  echo "$discord_webhook" > "$HOME/vpn_config/discord_webhook.txt"
  
  # Créer le script shell pour la mise à jour de l'adresse IP
...
if [ ! -f "\$PIVPN_CONFIG" ]; then
    echo "Erreur : Fichier de configuration PiVPN introuvable." >> "\$LOG_FILE"
    exit 1
fi
...
if [ "\$current_public_ip" == "\$old_public_ip" ]; then
    echo "Adresse IP publique inchangée. Aucune action requise." >> "\$LOG_FILE"
    exit 0
fi
...
echo "Mise à jour de la configuration du serveur PiVPN..." >> "\$LOG_FILE"
sudo sed -i "s/\$old_public_ip/\$current_public_ip/g" "\$PIVPN_CONFIG"

# Mettre à jour les configurations des clients
echo "Mise à jour des configurations des clients..." >> "\$LOG_FILE"
for client_config in "\$CLIENT_CONFIGS_DIR"/*.conf; do
    if [ -f "\$client_config" ]; then
        ...
        echo "Mise à jour de la configuration pour \$client_name" >> "\$LOG_FILE"
    fi
done

# Redémarrer le service Wireguard
echo "Redémarrage du service Wireguard..." >> "\$LOG_FILE"
...
if [ -f "\$VPN_CONFIG_DIR/discord_webhook.txt" ]; then
    ...
    echo "Fichier webhook Discord non trouvé." >> "\$LOG_FILE"
fi
...
EOL
  ...
  echo -e "\n${LIGHT_BLUE}Script de mise à jour IP créé avec succès et cronjob configuré !${NC}"
  echo "Le script s'exécutera toutes les 10 minutes pour vérifier et mettre à jour l'adresse IP publique."
